...
export default function AircraftCard({ aircraft }: { aircraft: AircraftWithMedia }) {
  const hero = pickAircraftHero(aircraft);
  return (
    <Link
      href={`/aviation/${aircraft.slug}`}
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-ink-800">
        {hero ? (
          <Image
            src={hero.url}
            alt={hero.alt_text || `${aircraft.year} ${aircraft.manufacturer} ${aircraft.model}`}
            fill
      ...
      <div className="p-6 lg:p-8">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-widest2 text-bronze-300">
            {"— "}{AIRCRAFT_CATEGORY_LABEL[aircraft.category]} &middot; {aircraft.year}
          </p>
          <h3 className="mt-3 font-display text-2xl md:text-3xl leading-[0.95] tracking-tightest text-bone group-hover:text-bronze-200 transition-colors duration-500 ease-luxury">
            {aircraft.manufacturer} <span className="italic">{aircraft.model}</span>
          </h3>
        </div>
