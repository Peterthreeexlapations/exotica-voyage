...
import { pickYachtHero, yachtDisplayRate } from "@/lib/yachts";
import { formatRate } from "@/lib/format";
import { YACHT_CATEGORY_LABEL } from "@/lib/types";

export default function YachtCard({ yacht }: { yacht: YachtWithMedia }) {
  const hero = pickYachtHero(yacht);
  const rate = yachtDisplayRate(yacht);
  return (
    <Link
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-ink-800">
        {hero ? (
          <Image
            src={hero.url}
            alt={hero.alt_text || `${yacht.year} ${yacht.builder} ${yacht.model}`}
            fill
        ...
        <div className="absolute top-4 right-4 border border-bronze-300/60 bg-ink-950/70 px-3 py-1.5 backdrop-blur-sm">
          <span className="font-mono text-[10px] uppercase tracking-widest2 text-bronze-300">
            {yacht.length_ft.toFixed(0)} ft
          </span>
        </div>
      </div>
      ...
      <div className="p-6 lg:p-8">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-widest2 text-bronze-300">
            {"— "}{YACHT_CATEGORY_LABEL[yacht.category]} &middot; {yacht.year}
          </p>
          <h3 className="mt-3 font-display text-2xl md:text-3xl leading-[0.95] tracking-tightest text-bone group-hover:text-bronze-200 transition-colors duration-500 ease-luxury">
            {yacht.builder} <span className="italic">{yacht.model}</span>
          </h3>
        </div>
        ...
        <div className="mt-6 flex items-end justify-between border-t border-ink-700 pt-4">
          <div>
            ...
            <p className="mt-1 font-display text-xl text-bone">
              {formatRate(rate.amount)}
              <span className="font-mono text-[11px] uppercase tracking-widest2 text-bone/40 ml-2">/ {rate.period}</span>
            </p>
          </div>
